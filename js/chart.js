...
var padding = 2;
var nodes = [];
var force, node, data, maxVal,prevIm;
var imgCounter = 0;
var brake = 0.2;
...
var fill = d3.scale.ordinal().range(["#f000f0", "#00e0e0", "#000000"]);
...
function amountType() {
	...
	force.gravity(0)
		.friction(0.9)
		.charge(function(d) { return -Math.pow(d.radius, 2.0) / 2; })
        .on("tick", amounts)
        .start();
}
...
function moveToCentre(alpha) {
	return function(d) {
		...
		d.y += (centreY - 117 - d.y) * (brake + 0.06) * alpha * 1.2;
	};
}
...
function moveToAmounts(alpha) {
	return function(d) {
		...
		d.x += (centreX - d.x) * (brake + 0.02) * alpha * 1.2;
		d.y += (centreY - d.y) * (brake + 0.02) * alpha * 1.1;

		d.x += (centreX - d.x) * (brake + 0.06) * alpha * 1.0;
...
function updateHistory(d, imageLastViewed) {   
    
    if (imageLastViewed == prevIm)
        return;
    
    imgCounter++;
    prevIm = imageLastViewed;
    
	var img = document.createElement("img"); 
    
    img.src = imageLastViewed;
    img.style.height = '50px';
    img.style.width = '50px';
    img.style.paddingLeft = '24px';
    img.style.paddingTop = '15px';   
    
    var src = document.getElementById("border");
    ...
    if (imgCounter > 8) 
    {   
        src.removeChild(src.childNodes[7]);            
    }
...
function mouseover(d, i) {
    ...
    mosie.on("click", function(d){
        window.open('http://www.google.com.pk/search?btnG=1&pws=0&q=' + donor, '_blank');
    });    
    
    responsiveVoice.speak("donor is " + donor);
    responsiveVoice.speak("with ammount of " + comma(amount) + "british pounds");
    
    updateHistory(d, imageFile, donor);
}
